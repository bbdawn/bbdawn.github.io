...
{% assign menus = "GPU,Infra,Openstack › Loadbalancer,Network" | split: "," %}
{% assign menu_names = "GPU,Rack Topology,Load Balancer,Network" | split: "," %}
{% assign menu_icons = "fas fa-microchip,fas fa-server,fas fa-network-wired,fas fa-globe" | split: "," %}
{% assign subcategory_order = "Features,Study,Hands-on,Troubleshooting,Projects" | split: "," %}
...
{% for i in (0..3) %}
  ...
  {% if cat_posts.size > 0 %}
  <div class="col-12">
    <div class="card" style="border-radius: 12px; border: 1px solid var(--card-border-color);">
      <div class="card-header" style="border-radius: 12px 12px 0 0; background: var(--card-bg);">
        <span style="font-weight: 700; font-size: 1rem;">
          <i class="{{ menu_icons[i] }} fa-fw me-1"></i>{{ menu_names[i] }}
        </span>
        <span class="text-muted small ms-2">{{ cat_posts | size }} posts</span>
      </div>
      <div class="card-body p-3">
        {% for sub in subcategory_order %}
          ...
          {% if sub_posts.size > 0 %}
          <div class="mb-3">
            <div class="text-muted mb-1" style="font-size: 0.78rem; font-weight: 600; text-transform: uppercase; letter-spacing: 0.05em;">{{ sub }}</div>
            {% for post in sub_posts %}
            <div class="py-1">
              <i class="far fa-file-alt fa-fw text-muted" style="font-size: 0.8rem;"></i>
              <a href="{{ post.url | relative_url }}" style="font-size: 0.875rem;">{{ post.title }}</a>
            </div>
            {% endfor %}
          </div>
          {% endif %}
